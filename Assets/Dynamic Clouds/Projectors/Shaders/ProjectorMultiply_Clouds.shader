...
Shader "Projector/Multiply_Clouds" {
	Properties {
...
float f_noise(float2 uv, float p, float details){
  float4 timed =  float4(uv+float2(_Time[0]*_WindSpeed_X,_Time[0]*_WindSpeed_Y),0.0,_Time[0]*_CloudAnimation);
				...
				for (float f = 0.0 ; f <= details ; f++ ){
       				float power = pow( 2, f );
        			res +=  abs(cnoise( float4( power * timed ) )/power);
    			}	
    			res = min(1,res);
    			res = max(0,res);
			...
			{
			float noise = f_noise(i.uvShadow*_Tiling,1,10);

				fixed4 texS = tex2Dproj (_ShadowTex, UNITY_PROJ_COORD(i.uvShadow))+noise;
				texS.a = 1.0-texS.a;
