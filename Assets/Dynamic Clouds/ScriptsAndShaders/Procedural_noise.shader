// URP port of "Procedural Clouds/PC_Noise"
// - Remove UnityCG/UnityLightingCommon
// - Use URP Core.hlsl
// - UnityObjectToClipPos -> TransformObjectToHClip
// - Optional URP fog

Shader "Procedural Clouds/PC_Noise_URP"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ====== (Simplex/Perlin) 4D noise utilities (원본 유지) ======
            ...
            float cnoise(float4 P)
            {
                ...
                return 2.2 * lerp(n_yzw.x, n_yzw.y, fade_xyzw.x);
            }

            float pnoise(float4 P, float4 rep)
            {
                // periodic version: 그대로 유지 (원본과 동일)
                float4 Pi0 = fmod(floor(P), rep);
                float4 Pi1 = fmod(Pi0 + 1.0, rep);
                Pi0 = mod289(Pi0);
                Pi1 = mod289(Pi1);
                float4 Pf0 = frac(P);
                float4 Pf1 = Pf0 - 1.0;
                ...
                float n0000 = dot(float4(gx00.x,gy00.x,gz00.x,gw00.x), Pf0);
                float n1000 = dot(float4(gx00.y,gy00.y,gz00.y,gw00.y), float4(Pf1.x, Pf0.yzw));
                float n0100 = dot(float4(gx00.z,gy00.z,gz00.z,gw00.z), float4(Pf0.x, Pf1.y, Pf0.zw));
                float n1100 = dot(float4(gx00.w,gy00.w,gz00.w,gw00.w), float4(Pf1.xy, Pf0.zw));
                float n0010 = dot(float4(gx10.x,gy10.x,gz10.x,gw10.x), float4(Pf0.xy, Pf1.z, Pf0.w));
                float n1010 = dot(float4(gx10.y,gy10.y,gz10.y,gw10.y), float4(Pf1.x, Pf0.y, Pf1.z, Pf0.w));
                float n0110 = dot(float4(gx10.z,gy10.z,gz10.z,gw10.z), float4(Pf0.x, Pf1.yz, Pf0.w));
                float n1110 = dot(float4(gx10.w,gy10.w,gz10.w,gw10.w), float4(Pf1.xyz, Pf0.w));
                float n0001 = dot(float4(gx01.x,gy01.x,gz01.x,gw01.x), float4(Pf0.xyz, Pf1.w));
                float n1001 = dot(float4(gx01.y,gy01.y,gz01.y,gw01.y), float4(Pf1.x, Pf0.yz, Pf1.w));
                float n0101 = dot(float4(gx01.z,gy01.z,gz01.z,gw01.z), float4(Pf0.x, Pf1.y, Pf0.z, Pf1.w));
                float n1101 = dot(float4(gx01.w,gy01.w,gz01.w,gw01.w), float4(Pf1.xy, Pf0.z, Pf1.w));
                float n0011 = dot(float4(gx11.x,gy11.x,gz11.x,gw11.x), float4(Pf0.xy, Pf1.zw));
                float n1011 = dot(float4(gx11.y,gy11.y,gz11.y,gw11.y), float4(Pf1.x, Pf0.y, Pf1.zw));
                float n0111 = dot(float4(gx11.z,gy11.z,gz11.z,gw11.z), float4(Pf0.x, Pf1.yzw));
                float n1111 = dot(float4(gx11.w,gy11.w,gz11.w,gw11.w), Pf1);
            ...
            float3 computeNormals(float h_A, float h_B, float h_C, float h_D, float h_N, float heightScale)
            {
                ...
                float3 average_n = (cross(va, vb) + cross(vb, vc) + cross(vc, vd) + cross(vd, va)) / -4.0;
                return normalize(average_n);
            }

            // Original f_noise adapted to URP time/params
            float f_noise(float2 uv, float p, float details)
            {
                float2 t = float2(_Time.y * _WindSpeed_X, _Time.y * _WindSpeed_Y);
                float4 timed = float4(uv + _Rnd.xy + t * _Tiling, 0.0, _Time.y * _CloudAnimation);

                float res = -1.2 + (_Density) * p;
                for (float f = 0.0; f <= details * (_Detalization); f += 1.0)
                {
            ...
            half4 Frag(Varyings i) : SV_Target
            {
                ...
                float n = f_noise(uv * _Tiling, 1.0, 4.0);
                half4 col = half4(n, n, n, n);
