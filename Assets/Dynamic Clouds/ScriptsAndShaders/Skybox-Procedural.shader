...
Shader "Skybox/Procedural_stars" {
Properties {
	[KeywordEnum(None, Simple, High Quality)] _SunDisk ("Sun", Int) = 2
	_SunSize ("Sun Size", Range(0,1)) = 0.04

	_SkySaturation("Sky Saturation", Range(0.1,1)) = 1

	_AtmosphereThickness ("Atmoshpere Thickness", Range(0,5)) = 1.0
	_SkyTint ("Sky Tint", Color) = (.5, .5, .5, 1)
	_GroundColor ("Ground", Color) = (.369, .349, .341, 1)
	_StarsDensity ("Stars Density",Range(0,1)) = 1
...
SubShader {
	...
	Pass {
		...
		v2f vert (appdata_t v)
		{
			...
			OUT.abs_p =mul(unity_ObjectToWorld, v.vertex);
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
		...
		#endif
			 //float4 tex =tex2D(_NightTex, IN.uv*2);
			float brightness =pow(rand(floor((float3(normalize(IN.abs_p).x,normalize(IN.abs_p).y,normalize(IN.abs_p).z))*100)/1),2)/4;
			 float star = min(step(1-_StarsDensity/50,rand(floor((float3(normalize(IN.abs_p).x,normalize(IN.abs_p).y,normalize(IN.abs_p).z))*350)/1))*brightness,.5);
			// if we did precalculate color in vprog: just do lerp between them
			IN.skyColor = lerp(IN.skyColor,(IN.skyColor.r+IN.skyColor.g+IN.skyColor.b)/3,1-_SkySaturation);
			col = lerp(IN.skyColor + max(0,(1-(IN.skyColor.r+IN.skyColor.g+IN.skyColor.b))*star), IN.groundColor, saturate(y));
