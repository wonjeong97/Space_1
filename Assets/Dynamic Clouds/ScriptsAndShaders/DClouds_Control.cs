...
public class DClouds_Control : MonoBehaviour {
	[Header("If more than 1, enter")]
	public int number_of_submeshes =5;
	...
	void Start () {
		...
		shadows.GetComponent<Projector>().material = shadows_material;//just making sure that material instance is used and runtime changes do not affect basic material(for some reason instances weren't created automatically in unity 5.3) 
...
*/
	...
	void MoveListUp(int n){ // moving the list on cell up after deletion
	...
	void OnGUI(){
		...
		cl_parameters_temp[15]  = Mathf.Round(GUI.HorizontalSlider(new Rect(200,55,130,20),cl_parameters_temp[15] ,0.1f,8f));
			...
			cl_parameters_temp[16]  =  GUI.Toggle(new Rect(200,70,250,20),cl_parameters_temp[16]==1?true : false,"Additional Noise: ")?1:0;//Mathf.Round(GUI.HorizontalSlider(new Rect(200,85,130,20),cl_parameters_temp[16] ,0f,1f));
